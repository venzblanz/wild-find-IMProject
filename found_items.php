...
if ($selected_cat > 0) {
    $stmt = $connection->prepare(
        "SELECT p.post_id, p.itemName, p.publicDescription, p.currentStatus, p.dateReported,
         ...
         WHERE p.type = 'FOUND' AND p.category_id = ?
...
<div class="page-wrapper">
    ...
    <div class="category-filter">
        <a href="found_items.php" class="cat-btn <?= $selected_cat === 0 ? 'active' : '' ?>">All</a>
        <?php foreach ($cats as $cat): ?>
            <a href="found_items.php?category_id=<?= $cat['category_id'] ?>"
